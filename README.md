...
> Phiên bản hiện tại: **2.15.1** — chọn folder lưu · progress video live · multi-LLM + Grok · …  
> Hướng dẫn đầy đủ: **[docs/HUONG_DAN_FEATURES_WORKFLOWS.md](docs/HUONG_DAN_FEATURES_WORKFLOWS.md)** · `python app/main.py --version`  
...
|--------------|---------|
| **Windows** | Double-click **`SkoolDownloader.cmd`** · hoặc shortcut Desktop (chạy `Nâng cao\Tao shortcut Desktop.cmd` một lần) |
| **macOS** | Double-click **`SkoolDownloader.command`** · hoặc **Desktop → Skool Downloader** · hoặc **`~/Applications/Skool Downloader.app`** |
| **Linux** | `bash SkoolDownloader.command` hoặc desktop entry sau khi cài shortcut |
...
```
Skool-Downloader/
├─ SkoolDownloader.cmd             ← ⭐ Windows: BAM PHAT LA CHAY
├─ SkoolDownloader.command         ← ⭐ macOS: double-click mo app
...
→ **Windows:** chỉ cần `SkoolDownloader.cmd`. **macOS:** `SkoolDownloader.command` hoặc shortcut Desktop. Folder `Nâng cao/` cho CLI.
...
## 1b. Giao diện (SkoolDownloader) — cách dùng chính

Double-click **`SkoolDownloader.cmd`** / **`.command`** → mở app. Lần đầu trên máy mới tự cài môi trường rồi mở. Màn chính: **Dashboard**.

### Quy trình nhanh (khuyến nghị)

```
1. Chọn thư mục lưu (Dashboard → «Chọn…» → «Lưu»)
2. ➕ Thêm khóa mới  (hoặc ▶ Mở khóa đã có)
3. Dump chương từ Skool (trình duyệt trong app)
4. ⬇ Tải toàn bộ / theo chương / theo bài
5. Xem progress live: khóa · folder · video · bài đang/xong/chờ
6. (Tuỳ chọn) Phụ đề · Chat RAG · Cloud · LLM Prompt
```

### Chọn thư mục lưu (output / BASE)

| Cách | Thao tác |
|------|----------|
| **GUI (dễ)** | Dashboard → ô **📁 Thư mục lưu khóa** → **Chọn…** → **Lưu** |
| **Doctor** | Sidebar 🩺 Doctor → **Chọn…** / **Lưu** |
| **CLI** | `python app/doctor.py --set-base "D:/SkoolData"` |
| **Env** | `SKOOL_BASE=/path/to/folder` |

Khóa lưu tại: **`<BASE>/courses/<Tên khóa>/`** (mỗi bài: `description.md`, `video.*`, phụ đề, `notes.md`…).

### Thông tin video trên giao diện

| Màn hình | Hiển thị |
|----------|----------|
| **Dashboard KPI** | Video đã tải `x/y` · dung lượng tổng · cảnh báo token/fail |
| **Card mỗi khóa** | `Video x/y · N chương · dung lượng · còn Z` + progress bar |
| **Trình tải (manager)** | Tổng video, dung lượng, nguồn (YouTube/Loom/native); chương `▶ done/total` + size; bài: ✓/⏳/• · size · host |
| **Khi đang tải** | Panel live: **Khóa · Folder · Video x/y · Bài hiện tại · progress bar · 3 cột Đã xong / Đang / Chuẩn bị** |
| **Step “Đang tải…”** | % lớn · ETA · bài đang xử lý · danh sách chương |

**Trình tải theo chương/bài:** Mở khóa → **▸** xem bài → **⬇ Chương** / **⬇** từng bài / **⬇ Tải toàn bộ** · **■ Dừng** bất cứ lúc nào. CLI: `--chapter`, `--lesson`.

**Sidebar (nâng cao):**

| Tính năng | Mô tả | CLI / ghi chú |
|-----------|-------|----------------|
| **⌂ Dashboard** | Chọn folder lưu · KPI video · card khóa (Video x/y · size · badge) · progress live khi đang tải | `progress.scan_all` |
...
| Tính năng | Mô tả |
|-----------|-------|
| **Tải tiếp khóa dở** | Dashboard / manager hiện `Video x/y · dung lượng · còn N`; chỉ tải phần thiếu |
...
2. `app/.settings.json` → `"skool_base"` (sau khi bấm **Lưu** trên Dashboard)
3. Tự nhận: có `courses/` cạnh repo **hoặc** trong repo
...
**Khuyến nghị:** dùng nút **Chọn…** trên Dashboard (không cần nhớ CLI). Cũng có ở **🩺 Doctor**.
...
**Không cần làm gì riêng** — cứ double-click **`SkoolDownloader.cmd`**, lần đầu nó tự tạo venv + cài thư viện + ffmpeg rồi mở giao diện. (Nếu máy chưa có Python, nó hiện thông báo bảo tải Python 3.11+ ở python.org, nhớ tick *Add to PATH*.)

> 💡 Máy hiện tại **đã có sẵn** `..\whisper\venv` đầy đủ (yt-dlp + faster-whisper + model) → `SkoolDownloader.cmd` tự dùng venv đó, mở giao diện tức thì.
>
...
> Cách nhanh nhất là dùng giao diện (`SkoolDownloader.cmd`). Các ví dụ `run.cmd` dưới đây nay nằm ở **`Nâng cao\Tai bang dong lenh.cmd`** (cùng tham số):
