...
`run.ps1` → `main.py` chạy lần lượt (có **preflight** kiểm tra môi trường ở đầu):

| Bước | Module | Việc |
|------|--------|------|
| *preflight* | [preflight.py](preflight.py) | Kiểm tra Python/Node/ffmpeg/yt-dlp/đĩa/JSON → PASS/WARN/FAIL. FAIL thì dừng |
| `folders`    | [folders.py](folders.py)    | Dựng cây `NN - Tên` cho từng chương/bài (tự tạo cả folder **chương** cho khóa mới) |
| `extras`     | [extras.py](extras.py)      | Ghi `description.md` + tải `resources/` *(link resource hết hạn sau **8h**)* |
| `videos`     | [videos.py](videos.py)      | Tải video → `video.<ext>` (yt-dlp). 2 lượt: **native trước** *(token **24h**)*, rồi Loom/YouTube. **Phân loại lỗi tự động** |
| `transcribe` | [transcribe.py](transcribe.py) | *(tùy chọn `--transcribe`)* faster-whisper → `video.txt` + `video.srt` |
| `audit`      | [audit.py](audit.py)        | Đối chiếu JSON ↔ file thực, xuất `video_audit.txt` |
...
```powershell
.\run.cmd --course "<Tên khóa>"               # hoặc: .\run.ps1 --course "<Tên khóa>"
...
python preflight.py --course "X"               # chỉ kiểm tra môi trường
```
...
.\uninstall_transcribe_task.ps1                          # gỡ khi xong
```
...
Chạy tay (không cần task): `.\run_transcribe_watch.ps1 --all` (hoặc thêm `--once` để làm 1 lượt rồi thoát).

> Whisper chỉ ra **text đúng ngôn ngữ gốc** (tiếng Anh). Muốn **phụ đề tiếng Việt** cần thêm bước dịch máy riêng cho file `.srt`.

---

## 7. Cấu hình ([config.py](config.py))
